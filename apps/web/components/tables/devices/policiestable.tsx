...
export function DevicesPoliciesTable({device}: {device: Device}) {
    const [columnFilters, setColumnFilters] = useState<{ id: string, value: string }[]>([]);
    const [policies, setPolicies] = useState<(Policy)[]>([]);
    useEffect(() => {
        if (!device || policies.length > 0) return;
        setPolicies(device.groups.flatMap(group => group.group.policyAssignments.map(policy => policy.policy)));
    }, [device, policies]);
