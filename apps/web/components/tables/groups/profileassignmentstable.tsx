...
export function GroupProfilesByAssignmentTable({ groupId }: { groupId: string }) {
    ...
    useEffect(() => {
        if (!profiles.loaded) {
            fetch("/api/v1/profiles", {
                ...
                setProfiles({ data: data.filter((profile: EnrolmentProfile) => profile.assignments.some((assignment: any) => assignment.groupId == groupId)), loaded: true });
            });
        }
    }, [profiles.loaded]);
