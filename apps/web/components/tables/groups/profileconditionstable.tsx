...
export function GroupProfilesByConditionTable({ groupId }: { groupId: string }) {
    ...
    useEffect(() => {
        if (!profiles.loaded) {
            fetch("/api/v1/profiles", {
                ...
                setProfiles({ data: data.filter((profile: EnrolmentProfile) => profile.conditions.some((condition: any) => condition.groupId == groupId)), loaded: true });
            });
        }
    }, [profiles.loaded]);
