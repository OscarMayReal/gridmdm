...
import { AppPolicy, Device, EnrolmentProfile, Policy } from "@repo/database";
import { GenericTable } from "@/components/generictable";
import { Input } from "@/components/ui/input";
import { useRouter } from "next/navigation";
import { Empty, EmptyDescription, EmptyMedia, EmptyTitle } from "@/components/ui/empty";
import { LayoutGridIcon, ListXIcon, PlusCircleIcon, PlusIcon, ShieldXIcon, XIcon } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Dialog, DialogClose, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, DialogTrigger } from "@/components/ui/dialog";
import { Field, FieldContent, FieldDescription, FieldLabel } from "@/components/ui/field";
import { Separator } from "@/components/ui/separator";

export function GroupAppPoliciesTable({ groupId }: { groupId: string }) {
    const [policies, setPolicies] = useState<{ data: AppPolicy[], loaded: boolean }>({ data: [], loaded: false });
    const [columnFilters, setColumnFilters] = useState([]);
    useEffect(() => {
        if (!policies.loaded) {
            fetch("/api/v1/apps/policies", {
                headers: {
                    "Content-Type": "application/json",
                },
                credentials: "include"
            }).then(res => res.json()).then(data => {
                setPolicies({ data: data.filter((policy: AppPolicy) => policy.assignments.some((assignment: any) => assignment.groupId === groupId)), loaded: true });
            });
        }
    }, [policies.loaded]);
        ...
        <>
            ...
            <div className="bg-white rounded-md border-1 border-[#e4e4e7] overflow-hidden">
                <GenericTable fallback={<Empty className="flex flex-col gap-2">
                    ...
                    <EmptyTitle>No app policies found</EmptyTitle>
                    <EmptyDescription>App policies are used to configure apps for devices. Create a new app policy to get started.</EmptyDescription>
...
function CreatePolicyDialog({ onCreate }: { onCreate: (policy: Policy) => void }) {
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Create App Policy</DialogTitle>
                    <DialogDescription>
                        App policies are used to configure apps for devices.
                    </DialogDescription>
                </DialogHeader>
                <Separator />
                <Field>
