...
export function GroupPoliciesTable({ groupId }: { groupId: string }) {
    ...
    useEffect(() => {
        if (!policies.loaded) {
            fetch("/api/v1/policies", {
                ...
                setPolicies({ data: data.filter((policy: Policy) => policy.assignments.some((assignment: any) => assignment.groupId === groupId)), loaded: true });
            });
        }
    }, [policies.loaded]);
