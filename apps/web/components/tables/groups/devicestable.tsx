...
export function GroupDevicesTable({groupId}: {groupId: string}) {
    ...
    useEffect(() => {
        if (!devices.loaded) {
            fetch("/api/v1/devices", {
                ...
                setDevices({ data: data.filter((device: Device) => device.groups.some((group: any) => group.groupId === groupId)), loaded: true });
            });
        }
    }, [devices.loaded]);
