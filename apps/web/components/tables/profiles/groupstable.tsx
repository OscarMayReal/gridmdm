...
import { Device, EnrolmentProfile, EnrolmentProfileGroupAssignment, KeyStoneGroup, Policy, PolicyAssignment } from "@repo/database";
...
export function GroupsAssignmentsTable({ profile, refresh }: { profile: EnrolmentProfile | null, refresh: () => void }) {
    const [columnFilters, setColumnFilters] = useState([]);
    const table = useReactTable({
        data: profile?.assignments as EnrolmentProfileGroupAssignment[],
        state: {
            columnFilters,
        },
        columns: [
            ...
            <DialogContent>
                ...
                <Field>
                    ...
                    <FieldContent>
                        <Select value={groupId} onValueChange={setGroupId}>
                            ...
                            <SelectContent>
                                {groups.map((group) => (
                                    <SelectItem key={group.id} value={group.id} disabled={profile.assignments.some((assignment) => assignment.groupId === group.id)}>
                                        {group.name}
                                    </SelectItem>
                                ))}
                            </SelectContent>
                ...
                <DialogFooter>
                    ...
                    <Button variant="default" onClick={() => {
                        fetch("/api/v1/profiles/" + profile?.id + "/assignments", {
                            method: "POST",
                            headers: {
